...
public class PssGui extends JFrame implements Utility, PSSbehaviours {
	...
	**/
	...
	public String getStringer(JTextField field1){
		String stringer = null;
		stringer = field1.getText();
	...
	public String getStringer(JComboBox box1){
		String stringer = null;
		stringer = (String) box1.getSelectedItem();
		return stringer;
		
	}
	public String getStringer(JTextArea area1){
		String stringer = null;
		stringer = (String) area1.getText();
		return stringer;
		
	}
	
	public Color getCurColor(JTextField field1){
		Color curColor;
		curColor = field1.getBackground();
		return curColor;	
	}

	
	/**
	 * Method that copies the contents of the form to an object	
	 */
	
	public void copyDataToObject()
	{
		status.setDtg(currentDtg.getText());
		status.setStatusBackGround(unitId.getBackground());
		...
		status.setGemTOh(gemtInopCount.getSelectedItem().toString());
	...
	/**
	 * Method that converts the fireUnitData object to JSON
	...
	/**
	 * Method that write the JSON file to disk
...
/*//Copy Paste this block of code in for transmitting

*/

/* Use this for recieving/updating variables on the local machine after recieving transmission
currentDtg.setText(status.getDtg());
unitId.setBackground(status.getStatusBackGround());
...
gemCOhCount.setSelectedItem(status.getGemTOh());
